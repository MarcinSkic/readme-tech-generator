...
export function getCode(selectedTechList: TechInList[], iconSize: number, header: string, generateBadges: boolean, generateTable: boolean) {
	...
	for (let i=0; i<selectedTechList.length; i++) {
		...
		if (startsRow){
			if (generateTable) {
				let groupName = `row${nextGroupNumber}`;
				if (generateBadges){ 
